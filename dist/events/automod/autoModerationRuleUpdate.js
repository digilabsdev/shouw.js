...
async function Events(oldRule, newRule, client) {
    ...
    for (const command of commands) {
        ...
        if (command.channel?.includes('$') && command.channel !== '$') {
            channel = client.channels.cache.get((await index_js_1.Interpreter.run({
                ...
                client,
                Temporarily: { newRule, oldRule },
                user: client.users.cache.get(newRule.creatorId)
            }, { sendMessage: false }))?.result ?? '');
            guild = channel?.guild;
        }
        await index_js_1.Interpreter.run(command, {
            client: client,
            Temporarily: { oldRule, newRule },
            guild,
            channel,
            user: client.users.cache.get(newRule.creatorId),
            member: guild?.members.cache.get(newRule.creatorId)
        });
    }
}
